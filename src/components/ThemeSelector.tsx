...
import type { Theme, ThemeGroup } from '../lib/themes';
import { extractThemeFromHtml } from '../lib/themes';
...
export default function ThemeSelector({ activeTheme, onThemeChange, allThemes, themeGroups, onAddTheme, onRemoveTheme }: ThemeSelectorProps) {
    const [isThemeOpen, setIsThemeOpen] = useState(false);
    const [showBottomFade, setShowBottomFade] = useState(true);
    const scrollRef = useRef<HTMLDivElement>(null);
    const fileInputRef = useRef<HTMLInputElement>(null);
    const selectedThemeName = allThemes.find(t => t.id === activeTheme)?.name;
    ...
    const pillThemes: Theme[] = pillThemeIds
        .map(id => allThemes.find(theme => theme.id === id))
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            const theme = extractThemeFromHtml(htmlString, fileName);
            onAddTheme(theme);
    ...
    return (
        <div className="flex items-center flex-wrap gap-2 lg:gap-4 px-4 lg:px-6 py-3 border-r border-transparent md:border-[#00000015] md:dark:border-[#ffffff15] shrink-0">
            ...
            <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-[13px] font-medium bg-[#0066cc]/10 dark:bg-[#0a84ff]/15 text-[#0066cc] dark:text-[#0a84ff] hover:bg-[#0066cc]/18 dark:hover:bg-[#0a84ff]/25 transition-all shrink-0"
            >
                <Upload size={13} />
                <span className="hidden sm:inline">提取样式</span>
            </button>
            ...
            <div className="relative shrink-0">
                ...
                >
                    {isInDropdown ? selectedThemeName : `全部 ${allThemes.length} 款`}
                        ...
                        <>
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between px-5 pt-4 pb-2">
                                    <span className="text-[15px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7]">选择排版风格 · {allThemes.length} 款</span>
                                    <button
                                        onClick={() => setIsThemeOpen(false)}
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                                                {group.themes.map(theme => (
                                                    <button
                                                        ...
                                                        className={`group relative flex flex-col items-start gap-1.5 p-3 rounded-xl text-left transition-all
                                                            ...
                                                            <div className="flex items-center gap-1">
                                                                {isCustomTheme(theme.id) && (
                                                                    <span
                                                                        ...
                                                                        className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-red-100 dark:hover:bg-red-900/30 transition-all cursor-pointer"
                                                                    >
            ...
            <div className="hidden lg:flex items-center ml-4 pl-4 border-l border-[#00000015] dark:border-[#ffffff15]">
                <p className="text-[13px] text-[#86868b] dark:text-[#a1a1a6] font-medium tracking-wide truncate max-w-[300px] xl:max-w-[450px]">
                    <span className="text-[#1d1d1f] dark:text-[#f5f5f7] font-semibold mr-1">{allThemes.find(t => t.id === activeTheme)?.name}：</span>
                    {allThemes.find(t => t.id === activeTheme)?.description}
                </p>
            </div>
        </div>
    );
